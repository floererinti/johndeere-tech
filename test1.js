...
const cookie = n => decodeURIComponent((new RegExp('(?:^|; )' + n + '=([^;]+)').exec(document.cookie) || [])[1] || '');
...
const root = [...doc.querySelectorAll(String.raw`[wire\:snapshot]`)]
...
const token  = (/"csrf":"([A-Za-z0-9]{40})"/.exec(html) || [])[1]   // Livewire stashes it in window.livewireScriptConfig (no meta tag on Curricula)
...
let count = 0;
if (errs) {
  count = Array.isArray(errs) ? errs.length : Object.keys(errs).length;
}
